...
def create_user(user_id, username=None):
    ...
    c.execute("INSERT INTO users (user_id, balance, model, last_reset, username) VALUES (?, ?, ?, ?, ?)",
              (user_id, balance, REAL_MODEL, None, username))
    conn.commit()
    conn.close()
...
async def start_cmd(message: types.Message):
    ...
    user_history[user_id] = []  # сброс истории
...
async def show_menu(message: types.Message):
    ...
    text = (
        ...
        f"текущая модель: {DISPLAY_MODEL}\n"
        f"просто пиши свои запросы в чат."
    )
